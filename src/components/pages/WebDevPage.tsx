...
import { Globe, CheckCircle2, Sparkles, ArrowRight, ShieldCheck, Zap, Server } from 'lucide-react';
import { Language } from '../../types';
import { translations } from '../../data/translations';
import { WEB_CATEGORIES } from '../../data/content';

interface WebDevPageProps {
  lang: Language;
  onOpenInquiryWithCategory: (category: string) => void;
  onSelectCategoryDetail: (catTitle: string, serviceType: 'web') => void;
}
...
export const WebDevPage: React.FC<WebDevPageProps> = ({
  ...
  const features = [
    "Free Domain Guidance (1 Year)", "Hosting & DNS Configuration",
    "Enterprise SSL Certificate", "100% Mobile & Tablet Responsive",
    "Google SEO Friendly Architecture", "Luxury Apple-Level Modern UI/UX",
    "Secure Coding (XSS/CSRF Protection)", "Ultra-Fast Page Speed (<1.5s)",
    "1 Month Free Technical Support", "Minor Post-Launch Revisions Included"
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="relative rounded-3xl overflow-hidden p-8 sm:p-12 text-center max-w-5xl mx-auto space-y-6 border border-amber-500/30 shadow-xl bg-slate-950">
          <div className="absolute inset-0 opacity-30 mix-blend-luminosity">
            <img 
              ...
              onError={(e) => {
                (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=1600&q=85";
              }}
            />
          </div>
          ...
          <div className="relative z-10 space-y-4 max-w-3xl mx-auto">
            <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-amber-500/20 border border-amber-400/30 text-amber-300 text-xs font-bold backdrop-blur-md">
              ...
              <span>Enterprise Modern Web Architecture</span>
            </div>
            ...
            <p className="text-amber-100/80 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed font-sans">
              From sleek corporate portals and AI web tools to full-fledged e-commerce marketplaces — built with Next.js 16 and luxury styling.
            </p>
            ...
            <div className="pt-2">
              ...
              <p className="text-[11px] text-amber-200/70 mt-2">*Pricing depends upon project scope, custom features & backend requirements.</p>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-8 rounded-3xl border border-amber-500/30 shadow-lg">
          <h2 className="font-cinzel text-xl font-bold text-slate-900 mb-6 text-center">
            Standard Features Included in Every Website
          </h2>
        ...
        {/* 26 Web Categories Section */}
        <div className="space-y-6">
          <div className="text-center max-w-2xl mx-auto">
            <h2 className="font-cinzel text-2xl sm:text-3xl font-bold text-slate-900">
              Explore Website Categories
            </h2>
            <p className="text-xs text-slate-500 mt-1">Select any category to view full specs or request an instant quote.</p>
          </div>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed">
                    Custom architecture with tailored layout, mobile optimization, and admin controls.
                  </p>
                </div>

                <div className="mt-6 pt-4 border-t border-amber-500/15 flex items-center justify-between gap-2">
                  <button
                    onClick={() => onSelectCategoryDetail(cat, 'web')}
